...
import VenderInfo from "./vendorInfo.jsx";
const { TabPane } = Tabs;

const ProductDetail = () => {
  const { id } = useParams();
  const { fecthCourseDetail, courseDetail } = useCourse();

  useEffect(() => {
    if (id) {
      (async () => {
        await fecthCourseDetail(id);
      })();
    }
  }, [id]);
      ...
      <div className="container mx-auto p-4">
        <div className="flex">
          ...
          <div className="w-3/5 pl-4">
            ...
            <div className="mt-2">
              <span className="text-xl text-black ">{courseDetail.price}</span>
            </div>
            <div className="mt-2 ml-1">
              <p className="text-[#36174d] ">{courseDetail.description}</p>
            </div>
